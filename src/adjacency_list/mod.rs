...
fn is_square_on(square_id: usize, square_status_map: &Vec<SquareStatus>) -> bool {
    if let SquareStatus::Off = square_status_map[square_id] {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn correctly_identifies_off_squares() {
        let mock_grid = tests_common::get_mock_grid();
        let mock_square_statuses = tests_common::get_mock_squares(mock_grid);
        ...
        for i in 0..10 {
            assert_eq!(
                is_square_on(i, &mock_square_statuses),
        ...
        for i in 0..10 {
            assert_eq!(
                is_square_on(i + 90, &mock_square_statuses),
