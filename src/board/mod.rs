...
pub fn Board() -> Html {
    ...
    let start_square = use_state(|| None::<Element>);
    let end_square = use_state(|| None::<Element>);
    let is_first_render = use_mut_ref(|| true);

    let handle_click = {
        let start_square = start_square.clone();
        Callback::from(move |event: MouseEvent| {
            ...
            if current_target.as_ref().unwrap().id() == "" {
                return;
            }
            ...
            match mod_key {
                false => {
                    ...
                    start_square.set(Some(current_target.unwrap().into()));
                }
                true => {
                    current_target.as_ref().map(|t| t.set_class_name("bg-gray-900"));
                }
            }
        })
    };
                ...
                false => {
                    match mouse_button as i16 {
                        ...
                        RIGHT_BUTTON => {
                            ...
                            end_square.set(Some(current_target.unwrap().into()));
                        }
                        _ => {}
                    }
                }
    ...
    *is_first_render.borrow_mut() = false;
    set_start_square(&start_square);
    set_end_square(&end_square);
